...
import { CandidateListResponse } from '@/domain/candidate/schemas';
...
interface NewCandidatesPageProps {
  initialData: CandidateListResponse;
  initialQueryKey: string;
}

function NewCandidatesContent({ initialData, initialQueryKey }: NewCandidatesPageProps) {
  ...
  const handleSelect = (id: string) => {
    const params = new URLSearchParams({ candidateId: id });
    router.replace(`/live-session?${params.toString()}`, { scroll: false });
  };
...
export default function NewCandidatesPage({ initialData, initialQueryKey }: NewCandidatesPageProps) {
  return (
    <Suspense fallback={<div style={{ padding: '2rem' }}>Loading...</div>}>
      <NewCandidatesContent initialData={initialData} initialQueryKey={initialQueryKey} />
    </Suspense>
  );
}
